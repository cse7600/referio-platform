...
export default function ProgramDetailPage() {
  ...
  const handleCopyChannelLink = async (channel: string) => {
    const link = buildChannelLink(channel)
    if (!link) return
    await navigator.clipboard.writeText(link)
    setCopiedChannel(channel)
    toast.success(`${channel} 채널 링크가 복사되었습니다`)
      ...
      <div className="w-full pb-28">
        ...
        <div className="rounded-xl border overflow-hidden bg-white">
          ...
          <div className="p-5 sm:p-6">
            ...
            {/* 추천 링크 */}
            {isApproved && (
              <div className="mt-4 rounded-lg bg-green-50 border border-green-200 p-3 space-y-3">
                <p className="text-xs font-medium text-green-700">내 추천 링크</p>
