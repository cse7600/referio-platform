...
- [ ] Vercel Cron 기반 검색량 순차 갱신 (우선순위 기반, 일 5,000개 키워드)
...
일일 처리량:
  - 단일 계정: 1,000 req/day × 5 keywords/req = 5,000 keywords/day
  - 멀티 계정: N개 계정 × 5,000 = N × 5,000 keywords/day (§17 참조)
  - API 배치: 5개 키워드/요청
  - 요청 간 딜레이: 300ms (rate limit 방어)
  - 하루 실행 시간: 1,000 × 0.3s = ~5분 (단일 계정 기준)
```
...
### 7.3 100만 키워드 전체 갱신 소요 시간

| 시나리오 | 고유 키워드 수 | 일일 처리 | 전체 갱신 소요 |
|---------|-------------|----------|-------------|
| 단일 계정 (밀리 단독) | 22만 | 5,000/일 | 44일 |
| 단일 계정 (100만 키워드) | 80만 (중복 제거) | 5,000/일 | 160일 |
| 멀티 계정 3개 | 80만 | 15,000/일 | **54일** |
| 멀티 계정 5개 | 80만 | 25,000/일 | **32일** |
| 멀티 계정 10개 | 80만 | 50,000/일 | **16일** |
| 유료 API 전환 시 (단일) | 80만 | 50,000/일 | 16일 |
...
단일 네이버 검색광고 API 계정의 무료 한도(1,000 req/day)로는 100만 키워드 전체 갱신에 160일 소요. PO가 다수의 네이버 광고 계정을 보유하고 있으므로, 계정 로테이션으로 처리 속도를 N배 향상시킨다.

### 17.2 네이버 검색광고 API 한도

| 항목 | 값 |
|------|-----|
| 엔드포인트 | `GET /keywordstool` (RelKwdStat) |
| 무료 한도 | 계정당 약 1,000 req/day (공식 문서 명시 없음, 실측 기반) |
| 배치 크기 | 요청당 최대 5개 키워드 (hintKeywords) |
| 일일 처리량 | 1,000 req × 5개 = 5,000 키워드/계정/일 |
| Rate limit | 초당 약 5회 (공식 명시 없음, 300ms 간격 권장) |

**ToS 관련**: 네이버 검색광고 API는 계정별 독립 할당. 다중 계정 보유 자체는 금지 조항 없음. 단, 동일 IP에서 과도한 요청 시 IP 차단 가능성 있으므로 요청 간 딜레이 유지 필요.

### 17.3 계정 수별 처리 시간 테이블

기준: 고유 키워드 1,000,000개 (중복 제거 후 약 800,000개), 계정당 5,000개/일

| 계정 수 | 일일 처리량 | 전체 갱신 소요 | 단일 대비 단축 |
|---------|-----------|-------------|-------------|
| 1개 | 5,000/일 | **160일** | — |
| 2개 | 10,000/일 | **80일** | 2배 |
| 3개 | 15,000/일 | **54일** | 3배 |
| 5개 | 25,000/일 | **32일** | 5배 |
| 10개 | 50,000/일 | **16일** | 10배 |
| 20개 | 100,000/일 | **8일** | 20배 |

> 참고: is_featured 키워드(수백~수천 개)는 7일 주기로 항상 최신 유지되며 위 테이블은 전체 키워드 1회 순환 기준.
...
| 2.1 | 2026-04-08 | memo_public 공개/비공개 토글 추가 (§18), 멀티 API 계정 로테이션 설계 (§17), 환경변수 NAVER_API_ACCOUNTS JSON 배열 방식 확정 | cse7600 |
