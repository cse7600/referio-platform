...
- [x] 기존 한화비전 키퍼메이트 파트너들의 데이터가 유실 없이 유지된다 ← 아래 이슈 참고
...
### [이슈] 멀티 프로그램 전환 후 키퍼메이트 프로그램 파트너 탐색에서 미노출

| 항목 | 내용 |
|------|------|
| 발생일 | 2026-03-31 (저녁) |
| 현상 | 파트너 프로그램 탐색(`/dashboard/programs`)에서 키퍼메이트 프로그램이 보이지 않음 |
| 원인 | migration 022 적용 시 기존 `partner_programs` 레코드에 `program_id` 백필이 되지 않은 파트너 존재. `GET /api/partner/programs`가 `programs` 테이블(`is_public=true`, `is_active=true`)로 목록을 가져오는데, 일부 enrollment 매칭이 누락되어 파트너가 이미 참여 중인 프로그램을 "미신청" 상태로 인식하거나 목록에서 제외됨 |
| 데이터 상태 | Supabase DB 데이터는 정상 보존. `partner_programs` 레코드 전원 유지 |
| 조치 | Supabase 대시보드에서 키퍼메이트 `partner_programs` 레코드에 `program_id` 직접 연결 (2026-03-31 13:34 KST, 약 112건) |
| 결과 | 프로그램 탐색 정상화 확인 |
| 재발 방지 | 향후 신규 광고주 onboarding 시 `partner_programs`에 `program_id` 백필 여부 반드시 확인 |
