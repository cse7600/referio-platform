/**
 * send-bulk-email-keepermate.js
 *
 * 한화비전 키퍼메이트 파트너 전원에게 비밀번호 설정 이메일 일괄 발송
 *
 * 실행:
 *   node scripts/send-bulk-email-keepermate.js          ← 실제 발송
 *   node scripts/send-bulk-email-keepermate.js --dry-run ← 대상 목록만 출력 (발송 없음)
 */
...
const RESET_PATH = '/signup/keepermate';
const KEEPERMATE_ADVERTISER_ID = 'ab7da1e1-2bef-4065-8c84-88c037f2b4dc';

// 테스트 계정 제외
const EXCLUDE_EMAILS = ['cym@puzl.co.kr', '01047807600@naver.com'];

// 건당 딜레이 (ms) — Resend/Supabase rate limit 방어
const DELAY_MS = 300;
...
async function main() {
  ...
  const { data: programs, error } = await admin
    ...
    .select('referral_code, partners!inner(id, name, email, auth_user_id)')
    .eq('advertiser_id', KEEPERMATE_ADVERTISER_ID)
    .not('partners.email', 'in', `(${EXCLUDE_EMAILS.map((e) => `"${e}"`).join(',')})`);
  ...
  for (let i = 0; i < filtered.length; i++) {
    ...
    try {
      ...
      if (linkError) throw new Error(`링크 생성 실패: ${linkError.message}`);
      ...
      const res = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          html: buildEmailHtml(partner.name, actionLink, referral_code),
        }),
      });

      const result = await res.json();
      if (!res.ok) throw new Error(`Resend 오류: ${JSON.stringify(result)}`);

      results.ok.push({ name: partner.name, email: partner.email });
      process.stdout.write(`✅ 완료\n`);
    } catch (err) {
      results.failed.push({ name: partner.name, email: partner.email, reason: err.message });
  ...
  console.log('\n=== 발송 완료 ===');
  console.log(`✅ 성공: ${results.ok.length}명`);
